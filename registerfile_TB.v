...
module registerfile_TB;
    ...
    task check1; input [31:0] exp; begin
        if (dataOut1 !== exp) begin
            $display("FAIL out1=%h exp=%h", dataOut1, exp); errors=errors+1; end
    end endtask
    task check2; input [31:0] exp; begin
        if (dataOut2 !== exp) begin
            $display("FAIL out2=%h exp=%h", dataOut2, exp); errors=errors+1; end
    end endtask
    ...
    initial begin
        ...
        rdAddress1=5; #1; check1(32'hAB);
        // escreve 0xCD em r7
        wrAddress=7; dataIn=32'hCD; wr=1; @(negedge clk); wr=0;
        rdAddress2=7; #1; check2(32'hCD);
        // r0 sempre 0 (tentar escrever nao altera)
        wrAddress=0; dataIn=32'hFFFF; wr=1; @(negedge clk); wr=0;
        rdAddress1=0; #1; check1(32'h0);
        // reset assincrono limpa
        rdAddress1=5; reset=1; #1; check1(32'h0);
        if (errors == 0) $display("registerfile_TB: PASS");
        else             $display("registerfile_TB: %0d ERRORS", errors);
        $finish;
    end
endmodule
