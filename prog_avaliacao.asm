...
Loop:		lw $r1, grupo*0x700($r31)              ;		r1 ← Mem[i + Número do Grupo * 0250h] 
		...
		sw $r10, grupo*0x700+1023($r15)        ;		MemDados[ultima posição RAM] ←  [r10]
							;
...
Loop1: 		lw $r1, grupo*0x700($r31)              ;       	r1 ← Mem[i + Número do Grupo * 0250h] 
		addi $r31, 1($r31)                      ;		i = i + 1  
		                                        ;		NOP as bubble
		                                        ;		NOP as bubble
		add $r10, $r1, $r10                     ;		r10 ← r1 + r10 (cumulative sum of nth vector value)
		...
		bne $r31, $r30, Loop1                   ;		if r31 != r30 then PC ← Loop1 Address
       		mul $r20, $r10, $r13                    ;      		r20 ← sum * 0x00FF
                                                	;       	NOP as bubble
                                                	;      	 	NOP as bubble
		sw $r20, grupo*0x700+1023($r15)        ;		MemDados[ultima posição RAM] ←  [r10]
		                                        ;
